...
def sample_data(df, sample_len, sample_num, num_classes):
    ...
    train_x_name = '2smfcc_train_x.npy'
    train_y_name = '2smfcc_train_y.npy'
    test_x_name = '2smfcc_test_x.npy'
    test_y_name = '2smfcc_test_y.npy'
...
def load_train_test_data():
    train_x = np.load('2smfcc_train_x.npy')
    train_y = np.load('2smfcc_train_y.npy')
    test_x = np.load('2smfcc_test_x.npy')
    test_y = np.load('2smfcc_test_y.npy')
    return train_x, train_y, test_x, test_y

def train_model(train_x, train_y, test_x, test_y, model):
    LOGDIR = "logs"
    CPKT = "cpkt/cp-{epoch:04d}.ckpt"
...
def predict_model(test_x, test_y, num_classes):
    ...
    model = create_model(INPUTSHAPE, num_classes)
    # Restore the weights
    latest = tf.train.latest_checkpoint('cpkt')
    model.load_weights(latest)
    #onehot_test_y = tf.keras.utils.to_categorical(test_y, num_classes=num_classes)
    #loss, acc = model.evaluate(test_x, onehot_test_y, verbose=2)
